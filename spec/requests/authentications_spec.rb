...
RSpec.describe 'Authentication', type: :request do
  ...
  describe 'Authentication' do
    ...
    it 'authenticates user' do
      post base_route.to_s, params: { username: user.username, password: user.password }
      expect(response).to have_http_status(:created)
    end

    it 'generates error when password is not passed' do
      post base_route.to_s, params: { username: user.username }
    ...
    it 'raises error when user is not found' do
      post base_route.to_s, params: { username: 'cleber_machado', password: user.password }
      expect(response).to have_http_status(:not_found)
    end

    it 'raises error when password is wrong' do
      post base_route.to_s, params: { username: user.username, password: 'wrong_password' }
